...
function makeCards(nCards) {
  ...
  for (var i = 0; i < nCards; i++) {
    ...
    thisCard.nnotes = Math.floor(
      VS.getRandExcl(
        thisCardType.numberOfNotes[0],
        thisCardType.numberOfNotes[1]
      )
    );
...
var scoreSettings = (function() {
  var generateButton = document.getElementById("settings-generate"),
    radioSetting = new VS.RadioSetting(
...
var cueIndicator = VS.cueBlink(cueTriangle)
  ...
  .inactive(function(selection) {
    selection.style("fill-opacity", 0).style("opacity", 0);
  })
  .on(function(selection) {
    selection.style("fill-opacity", 1).style("opacity", 1);
  })
...
function goToCard(eventIndex, dur) {
  ...
  if (typeof eventIndex !== "undefined") {
    updateCardIndicator(eventIndex);
  }
}
