...
VS.PitchClassSettings.prototype.updatePreferences = function () {
  ...
  // Hide/show sections
  document.getElementById("pitch-classes-numbers-preferences").style.display =
    pitchClassesValue === "numbers" ? "block" : "none";
  document.getElementById("pitch-classes-names-preferences").style.display =
    pitchClassesValue === "names" ? "block" : "none";

  var preferValue = this.preferRadio.get();

  // Set to the first preference if switching pitch class display
  if (
    pitchClassesValue === "numbers" &&
    preferValue !== "te" &&
    preferValue !== "ab"
  ) {
    this.preferRadio.set("te");
  } else if (
    pitchClassesValue === "names" &&
    preferValue !== "sharps" &&
    preferValue !== "flats"
  ) {
