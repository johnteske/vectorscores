(function () {
  ...
  const layout = (function () {
    ...
    document
      ...
      .addEventListener("click", layout.hide, false);
  ...
  const control = (function () {
    ...
    function createStepCallback(steps) {
      return function () {
        var pointer = VS.clamp(
          VS.score.getPointer() + steps,
          0,
          VS.score.getLength() - 1
        );
    ...
    control.setStateFromPointer = function () {
      var state =
        VS.score.getPointer() === 0
          ? "firstStep"
          : VS.score.pointerAtLastEvent()
          ? "lastStep"
          : "step";
