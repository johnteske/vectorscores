...
loadDomThenTest("VS#getQueryString", htmlPath, (t, window) => {
  const { VS } = window;
  const url =
    "http://localhost:4000/vectorscores/scores/adsr/?parts=4&showall=on";

  t.equal(
    VS.getQueryString("parts", url),
    "4",
    "return value as string given query parameter that exists in url"
  );
  t.equal(
    VS.getQueryString("foo", url),
    null,
    "return null given query parameter that does not exist in url"
  );
...
loadDomThenTest("VS#makeQueryString", htmlPath, (t, window) => {
  ...
  t.equal(
    VS.makeQueryString({ a: "1", b: "2" }),
    "a=1&b=2",
    "return query string joined by '&'"
  );
