...
loadDomThenTest("VS.score events", htmlPath, (t, window) => {
  ...
  t.equal(
    VS.score.getLength(),
    scoreLength + 1,
    "adding an event to score to add one to score length"
  );
...
loadDomThenTest("VS.score play, pause, stop", htmlPath, (t, window) => {
  ...
  t.equal(
    VS.score.isPlaying(),
    false,
    "should not be flagged as playing on load"
  );
  ...
  t.equal(
    VS.score.isPlaying(),
    true,
    "should be flagged as playing when playing"
  );
  ...
  t.equal(
    VS.score.isPlaying(),
    false,
    "should not be flagged as playing when paused"
  );
  ...
  t.equal(
    VS.score.isPlaying(),
    false,
    "should not be flagged as playing when stopped"
  );
